...
{
    public class SocketClientUtilsFB
    {
        ...
        public static bool IsConnect_FB = false;
        ...
        {
            ServerReceiveSuccess = new SocketClientUtilsFB.receiveSuccess<ReceiveData>(Method);
        }
        ...
        /// 断开连接
        /// </summary>
        public void DisConnect_FB()
        {
            if (client_FB == null)
                return;

            if (!client_FB.Connected)
                return;

            try
            {
                client_FB.Shutdown(SocketShutdown.Both);
            }
            catch
            {
            }
        ...
        public bool IsConnectSocket_TQ()
        {
            ...
            return client_FB.Connected;
        }

        public byte[] buffer = new byte[1024 * 1024 * 3];
        private static Socket client_FB = null;
        ...
        public void StartClient()
        {
            ...
            {
                DisConnect_FB();
                IPAddress ipAddress = IPAddress.Parse(GetDataUtils.FBServerIP);
                IPEndPoint remoteEP = new IPEndPoint(ipAddress, GetDataUtils.FBServerPort);
                client_FB = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
                client_FB.SendTimeout = 10000;
                //client_TQ.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.NoDelay, true);

                var tcpKeepalive = new byte[12];
                client_FB.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.KeepAlive, true); // enable keep-alive
                ...
                client_FB.IOControl(IOControlCode.KeepAliveValues, tcpKeepalive, null); // set keep-alive parameter

                client_FB.BeginConnect(remoteEP, new AsyncCallback(ConnectCallBack), client_FB);
            }
            catch (Exception ee)
            {
                IsConnect_FB = false;
        ...
        private void ReceiveCallBack(IAsyncResult ar)
        {
            try
            {
                if (client_FB.Connected)
                {
                    ...
                    int bytesRead = client_FB.EndReceive(ar);
                    if (bytesRead > 0)
                    {
                        ...
                        client_FB.BeginReceive(buffer, 0, buffer.Length, 0, new AsyncCallback(ReceiveCallBack), this);//接收剩余的数据或者继续接收数据
                    }
                    else //如果接收到数据字符为0，代表接收完了
                    {
                        client_FB.BeginReceive(buffer, 0, buffer.Length, 0, new AsyncCallback(ReceiveCallBack), this);//继续接收数据
                    }
                }
                else
                {
        ...
        public void SendMessage(string DataBody)
        {
            try
            {
                ...
                client_FB.BeginSend(byteData, 0, byteData.Length, 0, new AsyncCallback(SendCallBack), this);
            }
            catch (Exception e)
            {
                IsConnect_FB = false;
        ...
        private void SendCallBack(IAsyncResult ar)
        {
            try
            {
                ...
                int bytesSend = client_FB.EndSend(ar);
