...
{
    public class SocketClientUtilsTWOJC
    {
        ...
        public static bool IsConnect_TWOJC = false;
        ...
        {
            ServerReceiveSuccess = new SocketClientUtilsTWOJC.receiveSuccess<ReceiveData>(Method);
        }
        ...
        /// 断开连接
        /// </summary>
        public void DisConnect_TWOJC()
        {
            if (client_TWOJC == null)
                return;

            if (!client_TWOJC.Connected)
                return;

            try
            {
                client_TWOJC.Shutdown(SocketShutdown.Both);
            }
            catch
            {
            }
        ...
        ///连接状态
        /// </summary>
        public bool IsConnectSocket_TWOJC()
        {
            if (client_TWOJC == null)
            {
                return false;
            }
            return client_TWOJC.Connected;
        }

        public byte[] buffer = new byte[1024 * 1024 * 3];
        private static Socket client_TWOJC = null;
        ...
        public void StartClient()
        {
            ...
            {
                DisConnect_TWOJC();
                IPAddress ipAddress = IPAddress.Parse(GetDataUtils.TWOServerIP);
                IPEndPoint remoteEP = new IPEndPoint(ipAddress, GetDataUtils.TWOServerPort);
                client_TWOJC = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
                client_TWOJC.SendTimeout = 10000;
                //client_TQ.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.NoDelay, true);

                var tcpKeepalive = new byte[12];
                client_TWOJC.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.KeepAlive, true); // enable keep-alive
                ...
                client_TWOJC.IOControl(IOControlCode.KeepAliveValues, tcpKeepalive, null); // set keep-alive parameter

                client_TWOJC.BeginConnect(remoteEP, new AsyncCallback(ConnectCallBack), client_TWOJC);
            }
            catch (Exception ee)
            {
                IsConnect_TWOJC = false;
        ...
        private void ReceiveCallBack(IAsyncResult ar)
        {
            try
            {
                if (client_TWOJC.Connected)
                {
                    ...
                    int bytesRead = client_TWOJC.EndReceive(ar);
                    if (bytesRead > 0)
                    {
                        ...
                        client_TWOJC.BeginReceive(buffer, 0, buffer.Length, 0, new AsyncCallback(ReceiveCallBack), this);//接收剩余的数据或者继续接收数据
                    }
                    else //如果接收到数据字符为0，代表接收完了
                    {
                        client_TWOJC.BeginReceive(buffer, 0, buffer.Length, 0, new AsyncCallback(ReceiveCallBack), this);//继续接收数据
                    }
                }
                else
                {
        ...
        public void SendMessage(string DataBody)
        {
            try
            {
                ...
                client_TWOJC.BeginSend(byteData, 0, byteData.Length, 0, new AsyncCallback(SendCallBack), this);
            }
            catch (Exception e)
            {
                IsConnect_TWOJC = false;
        ...
        private void SendCallBack(IAsyncResult ar)
        {
            try
            {
                ...
                int bytesSend = client_TWOJC.EndSend(ar);
