...
namespace _29_TCP异步通信之服务器_界面
{
    public class Net
    {
        public static ManualResetEvent allDone = new ManualResetEvent(false);
        private Socket listenerSocket;//Socket对象
        public static Dictionary<string, Socket> connectedSocket;//已连接上的Socket客户端对象
        ...
        public static event ConnectSuccess<IPEndPoint> serverConnectSuccess;
        ...
        public static void DeleteMethod()
        {
            if (serverReceiveSuccess != null)
            {
                ...
                foreach (Delegate d in dels)
                {
                    ...
                    serverReceiveSuccess -= d as receiveSuccess<string>;
                }
            }
        }

        public static void DeleteMethodByName(string name)
        {
            if (serverReceiveSuccess != null)
            {
                Delegate[] dels = serverReceiveSuccess.GetInvocationList();
        ...
        public static event receiveSuccess<string> serverReceiveSuccess;
        ...
        public void Connnect(string ip)
        {
            ...
            //hostPORT = 8085;//监听的端口
            hostPORT = GetDataUtils.ONESELFServerPort;//监听的端口
        ...
        private void AcceptCallback(IAsyncResult ar)//异步操作状态的接口
        {
                ...
                lock (connectedSocket)
                {
                    if (connectedSocket.ContainsKey(ip) == false)//不在连接的队列中
                    {
                        try
                        {
                            ...
                            log4netHelper.Info("检测到" + ip + "的连接...");
                            ConnectComplete(iep);//连接成功后，触发事件信号量
                        }
                        catch (Exception ex)
                        {
                            log4netHelper.Info("断开连接1------" + ex.ToString());
                            UpdateEvent();
            ...
            {
                UpdateEvent();
                log4netHelper.Info("断开连接2------" + ex.ToString());
                Console.WriteLine(ex.ToString());
            }
        }

        public static IPEndPoint iep = null;//用于客户端的IP和端口
        ...
        private void ReadCallback(IAsyncResult ar)
        {
            ...
            catch (Exception)
            {
                ...
                {
                    UpdateEvent();
                    log4netHelper.Info("断开连接3------" + ex.ToString());
                    Console.WriteLine(ex.ToString());
                }
                return;
            }
        ...
        private static object _lockReceive = new object();
        ...
        /// <param name="ip"></param>        
        public static bool SendDefault(string sendMessage, string ip)
        {
            bool rt = true;
            string senddData = sendMessage.Replace(" ", "");
            Console.WriteLine("发送数据--> " + senddData);
            log4netHelper.Info("发送数据-->" + senddData);
